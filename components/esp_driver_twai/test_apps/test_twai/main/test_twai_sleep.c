...
TEST_CASE("twai sleep retention", "[twai]")
{
    ...
    for (int power = 0; power < 2; power++) {
        ...
        for (int node = 0; node < SOC_TWAI_CONTROLLER_NUM; node++) {
            ...
            esp_rom_gpio_connect_in_signal(TEST_TX_GPIO, twai_controller_periph_signals.controllers[node].rx_sig, false);
            esp_rom_gpio_connect_out_signal(TEST_TX_GPIO, twai_controller_periph_signals.controllers[node].tx_sig, false, false);
